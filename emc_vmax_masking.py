...
class EMCVMAXMasking(object):
    ...
    def remove_and_reset_members(
        ...
        portgroupinstancename = (
            self.get_port_group_from_mv_instance(conn, mvInstanceName))
        if numVolInMaskingView == 1:
            ...
            smiversion = self.utils.get_smi_version(conn)
            if self.utils.SMI_VERSION_8 > smiversion:
                self._do_unmap_volume(
                    conn, volumeInstance.path, controllerConfigService,
                    portgroupinstancename, extraSpecs)
    ...
    def _do_unmap_volume(self, conn, volumeInstanceName,
        ...
        if volumeInstance['EMCIsMapped']:
            LOG.warning(_LE("Device %(deviceId)s is still mapped."),
                        {'deviceId': deviceId})
    ...
    def get_port_group_from_mv_instance(
        ...
        if len(portGroupInstanceNames) > 0:
            ...
            LOG.warning(_LE("No port group found in masking view %(mv)s"),
                        {'mv': maskingViewInstanceName})
