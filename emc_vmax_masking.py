...
class EMCVMAXMasking(object):
    ...
    def _validate_existing_masking_view(
            self, conn, maskingViewDict, maskingViewInstanceName, extraSpecs):
    ...
    def _create_or_get_initiator_group(
        ...
        if foundInitiatorGroupInstanceName is None:
            ...
            # Check first is the IG is not being used by another IQN
            # If it is, delete it(if it is not part of another MV) and
            # recreate it, with the correct IQN for that host
            recreate = self._verify_initiator_group(
    ...
    def _verify_initiator_group_from_masking_view(
        ...
        if (foundInitiatorGroupFromConnector !=
                foundInitiatorGroupFromMaskingView):
            LOG.warn(_LW("The initiator group from the connector "
                         "%(connector)s does not match the initiator group "
                         "from the masking view %(mv)s.")
                     % {'connector': foundInitiatorGroupFromConnector,
                        'mv': foundInitiatorGroupFromMaskingView})
            if foundInitiatorGroupFromMaskingView:
                if foundInitiatorGroupFromConnector is None:
                    initiatorGroupInstanceName = (
                        self._recreate_initiator_Group(
                            conn, controllerConfigService, igGroupName,
                            initiatorNames, storageSystemName, extraSpecs))
                    if initiatorGroupInstanceName is None:
                        msg = (_("Failed to re-create initiator group "
                               "%(igGroupName)s.") %
                               {'igGroupName': igGroupName})
                        LOG.error(msg)
                        raise exception.VolumeBackendAPIException(data=msg)
                ...
                self._recreate_masking_view(
                    conn, controllerConfigService, initiatorGroupInstanceName,
                    maskingViewName, storageSystemName, extraSpecs)

        return True

    def _recreate_masking_view(
            self, conn, controllerConfigService, initiatorGroupInstanceName,
            maskingViewName, storageSystemName, extraSpecs):
        """Recreate a masking view

        :param conn: connection the ecom server
        :param controllerConfigService: the controller configuration service
        :param initiatorGroupInstanceName: the initiator instance name
        :param maskingViewName: the masking view name
        :param storageSystemName: one or more hardware id instance names
        :param extraSpecs: extra specificiations
        """
        storageGroupInstanceName = (
        ...
        portGroupInstanceName = self._get_port_group_from_masking_view(
            conn, maskingViewName, storageSystemName)
        if (initiatorGroupInstanceName and
            ...
            msg = (_("Failed to re-create masking view "
                   "%(maskingViewName)s.") %
                   {'maskingViewName': maskingViewName})
            LOG.error(msg)
            raise exception.VolumeBackendAPIException(data=msg)

    def _recreate_initiator_Group(
            self, conn, controllerConfigService, igGroupName,
            initiatorNames, storageSystemName, extraSpecs):
        """Recreate an initiator group

        :param conn: connection the ecom server
        :param controllerConfigService: the controller configuration service
        :param igGroupName: the initiator group name (String)
        :param initiatorNames: one or more hardware id instance names
        :param storageSystemName: one or more hardware id instance names
        :param extraSpecs: extra specificiations
        """
        initiatorGroupInstanceName = None
        storageHardwareIDInstanceNames = (
            self._get_storage_hardware_id_instance_names(
                conn, initiatorNames, storageSystemName))
        if not storageHardwareIDInstanceNames:
        ...
        recreate = self._verify_initiator_group(
            ...
            extraSpecs)
        if recreate:
            initiatorGroupInstanceName = self._create_initiator_Group(
                conn, controllerConfigService, igGroupName,
                storageHardwareIDInstanceNames)
        return initiatorGroupInstanceName
    ...
    def remove_and_reset_members(
        ...
        if extraSpecs[ISV3]:
            ...
            if connector:
                ...
                warningnmessage = (_("Cannot get storage from connector"))
                LOG.warning(warningnmessage)
    ...
    def _do_unmap_volume(self, conn, volumeInstanceName,
                         controllerConfigService, portgroupinstancename,
                         extraSpecs):
        """Unmap the volume from the target ports
        ...
        if volumeInstance['EMCIsMapped']:
            self._unmap_volume(conn, controllerConfigService,
                               portgroupinstancename, deviceId, extraSpecs)
        volumeInstance = conn.GetInstance(volumeInstance.path)
        if volumeInstance['EMCIsMapped']:
            LOG.warn(_LW("Device %(deviceId)s is still mapped."),
                     {'deviceId': deviceId})
    ...
    def get_port_group_from_mv_instance(
            self, conn, maskingViewInstanceName):
        """Get the port group in a masking view.
    ...
    def _unmap_volume(self, conn, controllerConfigService,
                      portgroupinstancename, deviceId, extraSpecs):
        """Delete empty storage group.
        ...
        :param controllerConfigService: controller config service
        :param mvInstanceName: masking view instance name
        :param deviceid: device id e.g 00B88
        :param extraSpecs: extra specifications
        :returns: boolean
        """
        LOG.info(_LI("Unmapping device %(deviceId)s starting..."),
                 {'deviceId': deviceId})
        targetportids = []
        lunames = []
        lunames.append(deviceId)
        ...
        for iscsiendpointinstancename in iscsiendpointinstancenames:
            targetportid = (
                self.utils.get_target_port_id(conn,
                                              iscsiendpointinstancename))
            targetportids.append(targetportid)
        if len(targetportids) > 0:
            if self.provision.unmap_volume(conn, controllerConfigService,
                                           targetportids, lunames,
                                           extraSpecs):
                LOG.info(_LI("Unmapping device %(deviceId)s successful."),
                         {'deviceId': deviceId})
                return True
        LOG.warn(_LW(
            "Unable to get port(s) to unmap %(deviceId)s. "),
            {'deviceId': deviceId})
        return False
    ...
    def _last_volume_delete_initiator_group(
        ...
        if len(maskingViewInstanceNames) == 0:
            LOG.debug(
                ...
                % {'initiatorGroupName': initiatorGroupInstanceName})
            self._delete_initiators_from_initiator_group(
                conn, controllerConfigService, initiatorGroupInstanceName)
            deleted = self._delete_initiator_group(
                conn, controllerConfigService, initiatorGroupInstanceName,
                extraSpecs)
            return deleted
        else:
            LOG.warn(_LW("Initiator group %(initiatorGroupName)s is "
    ...
    def _delete_storage_hardware_id(self,
        ...
        """
        rc, __ = conn.InvokeMethod('DeleteStorageHardwareID',
                                   hardwareIdManagementService,
        ...
        if rc == 0:
            ...
            LOG.warn(_LW("Deletion of initiator path %(hardwareIdPath)s "
                         "is failed.") %
                     {'hardwareIdPath': hardwareIdPath})

    def _verify_initiator_group(
            self, conn, controllerConfigService, igGroupName, extraSpecs):
        """Verify that the initiator group is not already being used

        Delete the initiator group if it is being used by another
        IQN so long as it is not part of another masking view

        :param conn - connection to the ecom server
        :param controllerConfigService - controller config service
        :param igGroupName - The path of the initiator object
        :param inStorageHardwareIDInstanceNames - list of hardwareIds
        :param extraSpecs
        """
        initiatorGroupInstanceName = self.utils.find_initiator_group(
            conn, controllerConfigService, igGroupName)
        if initiatorGroupInstanceName:
            deleted = self._last_volume_delete_initiator_group(
                conn, controllerConfigService, initiatorGroupInstanceName,
                extraSpecs)
            return deleted
        else:
