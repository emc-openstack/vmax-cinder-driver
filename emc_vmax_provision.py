...
class EMCVMAXProvision(object):
    ...
    def _terminate_migrate_session(self, conn, volumeInstanceName):
        ...
        try:
            ...
            except_string = ('Migration was successful but update context '
                             'operation failed')
            if except_string in six.text_type(ex):
                LOG.debug("Session is already terminated.")
                rc = 0L
            else:
                LOG.error(_("Terminating session failed. "
                            "Exception: %s."), ex)
                rc = 1L
        return rc
