...
class EMCVMAXISCSIDriver(driver.ISCSIDriver):
    ...
    def initialize_connection(self, volume, connector):
        ...
        device_info = self.common.initialize_connection(
            volume, connector)
        try:
            iscsi_ip_addresses = device_info['iscsi_ip_addresses']
        ...
        iscsi_properties = self.smis_get_iscsi_properties(
            volume, connector, iscsi_ip_addresses, is_multipath)
    ...
    def smis_get_iscsi_properties(self, volume, connector, iscsi_ip_addresses,
                                  is_multipath):
        ...
        properties = {}
        if len(iscsi_ip_addresses) > 1 and is_multipath:
            properties['target_portals'] = ([t['ip'] + ":3260" for t in
                                             iscsi_ip_addresses])
            properties['target_iqns'] = ([t['iqn'].split(",")[0] for t in
                                          iscsi_ip_addresses])
            properties['target_luns'] = [lun_id] * len(iscsi_ip_addresses)
        properties['target_discovered'] = True
        properties['target_iqn'] = iscsi_ip_addresses[0]['iqn'].split(",")[0]
        properties['target_portal'] = iscsi_ip_addresses[0]['ip'] + ":3260"
