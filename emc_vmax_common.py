...
class EMCVMAXCommon(object):
    ...
    def create_volume_from_snapshot(self, volume, snapshot):
        ...
        if not extraSpecs[ISV3]:
            self._sync_check_and_break(
                snapshotInstance, snapshot['name'], extraSpecs)
    ...
    def _delete_volume(self, volume):
        ...
        if not extraSpecs[ISV3]:
            self._sync_check_and_break(volumeInstance, volumeName, extraSpecs)
    ...
    def _sync_check_and_break(self, volumeInstance, volumeName, extraSpecs):
        """Check if volume is part of a sync process.

        If the volume is part of a sync process (source or target)
    ...
    def _update_pool_stats(self, backendName, arrayInfo):
        ...
        if (arrayInfo['FastPolicy'] is not None and
            ...
            list_format = ['FAST', 'policy', arrayInfo['FastPolicy']]
            ...
            list_format = ['NON-FAST', 'pool', arrayInfo['PoolName']]
        LOG.info(_LI(
