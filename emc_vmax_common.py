...
class EMCVMAXCommon(object):
    ...
    def _unmap_lun(self, volume, connector):
        ...
        if configservice is None:
            exception_message = (_("Cannot find Controller Configuration "
                                   "Service for storage system %(storage_system)s")
    ...
    def extend_volume(self, volume, newSize):
        ...
        additionalVolumeSize = six.text_type(int(newSize) - int(originalVolumeSize))
    ...
    def update_volume_stats(self):
        ...
        if fastPolicyName is not None:
            ...
            LOG.debug(
                "No Fast policy for Array:%(arrayName)s backend:%(backendName)s"
                % {'arrayName': arrayName,
                   'backendName': backendName})
