...
class EMCVMAXCommon(object):
    ...
    def initialize_connection(self, volume, connector):
        ...
        if self.configuration.safe_get('use_multipath_for_image_xfer'):
            is_multipath = True
    ...
    def _get_correct_port_group(self, deviceInfoDict, storageSystemName):
        ...
        if ('controller' in deviceInfoDict and
            ...
            try:
                ...
                exception_message = ("Unable to get the name of "
                                     "the masking view")
                raise exception.VolumeBackendAPIException(
                    data=exception_message)
            ...
            exception_message = ("Cannot get the portgroup from "
                                 "the masking view")
            raise exception.VolumeBackendAPIException(
                data=exception_message)
    ...
    def update_volume_stats(self):
        ...
        if self.pool_info['is_v3']:
            (location_info, total_capacity_gb, free_capacity_gb,
             provisioned_capacity_gb, array_reserve_percent) = (
                self._update_srp_stats(self.pool_info))
        else:
            # This is V2.
            (location_info, total_capacity_gb, free_capacity_gb,
        ...
        if array_reserve_percent and (
            ...
            data['reserved_percentage'] = array_reserve_percent
        else:
            data['reserved_percentage'] = reservedPercentage

        return data

    def _update_srp_stats(self, pool_info):
        """Update SRP stats.
        ...
        """
        pool_info['slo'] = (
            self.utils.parse_slo_from_file(pool_info['config_file']))
        pool_info['workload'] = (
            self.utils.parse_workload_from_file(pool_info['config_file']))

        (totalManagedSpaceGbs, remainingManagedSpaceGbs,
         provisionedManagedSpaceGbs, array_reserve_percent) = (
            self.provisionv3.get_srp_pool_stats(self.conn, pool_info))
        ...
        LOG.info(_LI(
            ...
            {'poolName': pool_info['pool_name'],
             'arrayName': pool_info['serial_number'],
    ...
    def _create_cloned_volume(
        ...
        if self.utils.is_snapcopy_enabled(cloneVolume):
            extraSpecs[SNAPCOPY] = True
            LOG.info(_LI("Executing snap copy using copy on write. "
                         "We will not wait for a full sync."))
    ...
    def _sync_check(self, volumeInstance, volumeName, extraSpecs):
        ...
        syncName = self.utils.find_sync_sv_by_volume(
    ...
    def _update_pool_stats(
        ...
        :param pool_info: pool dictionary
        ...
        if fastPolicyName is not None:
            ...
            LOG.debug(
                ...
                "backend:%(backendName)s.",
                {'arrayName': pool_info['serial_number'],
                 'backendName': pool_info['backend_name']})

        storageSystemInstanceName = self.utils.find_storageSystem(
            self.conn, pool_info['serial_number'])
            ...
            (total_capacity_gb, free_capacity_gb, provisioned_capacity_gb,
             array_max_over_subscription) = (
                self.fast.get_capacities_associated_to_policy(
                    self.conn, pool_info['serial_number'],
                    fastPolicyName))
            list_format = ['FAST', 'policy', fastPolicyName]
            ...
            (total_capacity_gb, free_capacity_gb, provisioned_capacity_gb,
             array_max_over_subscription) = (
                self.utils.get_pool_capacities(self.conn,
                                               pool_info['pool_name'],
                                               pool_info['serial_number']))
            list_format = ['NON-FAST', 'pool', pool_info['pool_name']]

        LOG.info(_LI(
            "%(fastType)s: capacity stats for %(policyType)s %(poolName)s on "
            "array "
            "%(arrayName)s total_capacity_gb=%(total_capacity_gb)lu, "
            "free_capacity_gb=%(free_capacity_gb)lu, "
            "provisioned_capacity_gb=%(provisioned_capacity_gb)lu, "
            "max_over_subscription=%(array_max_over_subscription)lu."),
            {'fastType': list_format[0], 'policyType': list_format[1],
             'poolName': list_format[2],
             'arrayName': pool_info['serial_number'],
        ...
        location_info = ("%(arrayName)s#%(poolName)s#%(policyName)s"
                         % {'arrayName': pool_info['serial_number'],
                            'poolName': pool_info['serial_number'],
    ...
    def _create_clone_v2(self, repServiceInstanceName, cloneVolume,
        ...
        if metaHeadInstanceName is None:  # Simple volume.
            ...
            return self._create_replica_more_than_one_meta(
                repServiceInstanceName, sourceInstance, cloneVolume,
                sourceVolume, isSnapshot, extraSpecs)

    def _create_replica_more_than_one_meta(
            self, repServiceInstanceName, sourceInstance, cloneVolume,
            sourceVolume, isSnapshot, extraSpecs):
        """Create a replica where there is one than one meta
        ...
        if len(set(volumeCapacities)) == 1:
            ...
            LOG.debug(
                "Create V2 replica for meta members of different sizes.")
            baseTargetVolumeInstance = (
                self._create_replica_different_sized_metas(
                    volumeCapacities, sourceInstance, extraSpecs))
    ...
    def _create_replica_different_sized_metas(
            self, volumeCapacities, sourceInstance, extraSpecs):
        """Create a replica of different sized metas

        :param volumeCapacities: the replication service
        :param sourceInstance: the clone volume object
        :param extraSpecs: extra specifications
        :returns: int -- return code
        :returns: baseTargetVolumeInstance
        """
        ...
        for volumeSizeInbits in volumeCapacities:
            if baseTargetVolumeInstance is None:  # Create base volume.
                ...
                baseTargetVolumeInstance = (
                    self._modify_base_target_volume(
                        baseTargetVolumeInstance, volumeSizeInbits,
                        storageSystemName, baseVolumeDict, baseVolumeName,
                        extraSpecs))

        return baseTargetVolumeInstance

    def _modify_base_target_volume(
            self, baseTargetVolumeInstance, volumeSizeInbits,
            storageSystemName, baseVolumeDict, baseVolumeName,
            extraSpecs):
        """Create targets equal to the metas.

        :param baseTargetVolumeInstance: the base target volume instance
        :param volumeSizeInbits: the volume size in bits
        :param storageSystemName: the storage system name
        :param baseVolumeDict: the base volume dict
        :param baseVolumeName: the base volume name
        :param extraSpecs: extra specifications
        :returns: baseTargetVolumeInstance
        """
        ...
        unboundVolumeInstance = (
            self._create_and_get_unbound_volume(
                self.conn,
                storageConfigService,
                baseTargetVolumeInstance.path,
                volumeSizeInbits, extraSpecs))
        if unboundVolumeInstance is None:
            exceptionMessage = (_(
    ...
    def _create_v2_replica_and_delete_clone_relationship(
        ...
        fastPolicyName = extraSpecs[FASTPOLICY]
        if isSnapshot or extraSpecs[SNAPCOPY]:
            if fastPolicyName is not None:
                storageSystemName = sourceInstance['SystemName']
